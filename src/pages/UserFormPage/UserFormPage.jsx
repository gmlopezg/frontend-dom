    ...
    if (isEditing && id) { // Asegurarse de que 'id' existe
      const fetchUser = async () => {
        ...
        try {
          ...
          const response = await axios.get(`http://localhost:3001/api/usuarios/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        response = await axios.put(`http://localhost:3001/api/usuarios/${id}`, dataToUpdate, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
      } else {
        response = await axios.post('http://localhost:3001/api/usuarios/register', formData, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
      }
